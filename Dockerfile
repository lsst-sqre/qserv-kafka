...
FROM python:3.13.11-slim-trixie AS base-image
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    uv sync --python-preference=only-system --frozen --no-default-groups \
...
# Run the application.
CMD ["uvicorn", "--factory", "qservkafka.main:create_app", "--host", "0.0.0.0", "--port", "8080"]
