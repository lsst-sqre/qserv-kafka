...
def _slice_batch_to_rows(
    batch: pa.RecordBatch, offset: int
) -> list[tuple[Any, ...]]:
    sliced = batch.slice(offset, min(_RESULT_BATCH_SIZE, len(batch) - offset))
        ...
        while True:
            ...
            for offset in range(0, len(batch), _RESULT_BATCH_SIZE):
                rows = await asyncio.to_thread(
                    _slice_batch_to_rows, batch, offset
                )
                for row in rows:
                    yield row
